...
import { Clock, Users, AlertTriangle, Eye, CheckCircle } from 'lucide-react'
...
export default function HODDashboard({ profile, toast }) {
  ...
  const inFlight = reqs.filter(r => ['management_review', 'approved'].includes(r.status))
  const done     = reqs.filter(r => ['fulfilled', 'rejected'].includes(r.status))
  const urgent   = pending.filter(r => r.priority === 'Urgent').length

  const tabs = [
    { key: 'pending',   label: 'Pending',    count: pending.length },
    { key: 'inflight',  label: 'In Progress', count: inFlight.length },
    { key: 'done',      label: 'Completed',  count: done.length },
  ]

  const displayed = tab === 'pending' ? pending : tab === 'inflight' ? inFlight : done

  return (
    <div style={{ padding: '28px 32px', fontFamily: 'Inter, system-ui, sans-serif' }}>
      <div className="page-header">
        <div className="page-title">HOD Dashboard</div>
        <div className="page-sub">{profile.departments?.name} · Department Overview</div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 28 }}>
        {[
          { label: 'Pending Authorization', val: pending.length, icon: Clock,         color: 'var(--yellow)' },
          { label: 'Urgent',                val: urgent,         icon: AlertTriangle,  color: 'var(--red)' },
          { label: 'Total Dept Requests',   val: reqs.length,    icon: Users,          color: 'var(--blue)' },
        ].map(s => {
          const Icon = s.icon
          return (
      ...
      {loading ? (
        ...
        <div className="card empty-state">
          ...
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-2)' }}>
            {tab === 'pending' ? 'No pending authorizations' : tab === 'inflight' ? 'Nothing in progress' : 'No completed requests'}
          </div>
        </div>
      ) : (
        <div className="card" style={{ overflow: 'hidden' }}>
          <table className="table">
            <thead>
              <tr><th>Ref No.</th><th>Purpose</th><th>Requester</th><th>Items</th><th>Priority</th><th>Status</th><th>Date</th><th></th></tr>
            </thead>
            <tbody>
              {displayed.map(r => (
