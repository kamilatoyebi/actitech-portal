...
import { Clock, AlertTriangle, Eye, CreditCard } from 'lucide-react'

function Pill({ status }) {
  return <span className="pill" style={{ background: '#DBEAFE', color: 'var(--blue)' }}>{status === 'payment_review' ? 'Payment Review' : status}</span>
}

export default function AccountsDashboard({ profile }) {
  ...
  const totalCost = reqs.reduce((total, req) => total + (req.req_items || []).reduce((itemTotal, item) => itemTotal + Number(item.outsourcing_cost || 0), 0), 0)

  return (
    <div style={{ padding: '28px 32px', fontFamily: 'Inter, system-ui, sans-serif' }}>
      <div className="page-header"><div className="page-title">Accounts Dashboard</div><div className="page-sub">Payment review queue</div></div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 28 }}>
        {[
          { label: 'Pending Payment Review', val: reqs.length, icon: Clock, color: 'var(--blue)' },
          { label: 'Urgent', val: urgent, icon: AlertTriangle, color: 'var(--red)' },
          { label: 'Total Value', val: totalCost.toLocaleString(undefined, { maximumFractionDigits: 2 }), icon: CreditCard, color: 'var(--green)' },
        ].map(stat => {
      ...
      {loading ? <div>{[1, 2, 3].map(i => <SkeletonCard key={i} />)}</div> : reqs.length === 0 ? (
        <div className="card empty-state"><CreditCard size={36} color="var(--blue)" style={{ opacity: 0.4 }} /><div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-2)' }}>No payments awaiting review</div></div>
      ) : (
