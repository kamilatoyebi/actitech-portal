...
export default function AdminPanel() {
  ...
  const [formData, setFormData] = useState({
    full_name: '', email: '', role: 'employee', status: 'active', department_id: ''
  })
  ...
  async function handleSaveUser() {
    ...
    try {
      ...
      setShowModal(false)
      setEditingUser(null)
      setFormData({ full_name: '', email: '', role: 'employee', status: 'active', department_id: '' })
      loadData()
    } catch (error) {
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ background: C.bg, borderBottom: `1px solid ${C.border}` }}>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: C.muted, textTransform: 'uppercase', fontSize: 11 }}>Name</th>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: C.muted, textTransform: 'uppercase', fontSize: 11 }}>Email</th>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: C.muted, textTransform: 'uppercase', fontSize: 11 }}>Role</th>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: C.muted, textTransform: 'uppercase', fontSize: 11 }}>Status</th>
                    <th style={{ padding: '12px 16px', textAlign: 'center', fontWeight: 700, color: C.muted, textTransform: 'uppercase', fontSize: 11 }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {employees.map(emp => (
                    <tr key={emp.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{
                          ...
                          background: emp.role === 'admin' ? '#DBEAFE' : emp.role === 'manager' ? '#FEF3C7' : '#F1F5F9',
                          color: emp.role === 'admin' ? C.primary : emp.role === 'manager' ? '#B45309' : C.muted
                        }}>
                          {emp.role}
                        </span>
                      </td>
          ...
          <div>
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead>
                  <tr style={{ background: C.bg, borderBottom: `1px solid ${C.border}` }}>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: C.muted, textTransform: 'uppercase', fontSize: 11 }}>Timestamp</th>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: C.muted, textTransform: 'uppercase', fontSize: 11 }}>Admin</th>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: C.muted, textTransform: 'uppercase', fontSize: 11 }}>Action</th>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: C.muted, textTransform: 'uppercase', fontSize: 11 }}>Details</th>
                  </tr>
                </thead>
                <tbody>
              ...
              {auditLogs.length === 0 && (
                <div style={{ padding: '24px', textAlign: 'center', color: C.muted }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div>
                  ...
                  <input
                    ...
                    style={{
                      width: '100%', padding: '10px 14px', border: `1px solid ${C.border}`, borderRadius: 8,
                      fontSize: 13, background: C.bg, boxSizing: 'border-box', opacity: editingUser ? 0.6 : 1
                    }}
                  />
                </div>
